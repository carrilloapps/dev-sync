...
export class WindSurfAnalyzer {
			...
			try {
				...
				for (const entry of entries) {
					...
					if (entry.isDirectory()) {
						if (
							!['node_modules', '.git', 'dist', 'build', 'target', '.windsurf', '.vscode'].includes(
								entry.name
							)
						) {
							scanDir(fullPath);
	...
	private async extractSessionData(): Promise<SessionData | undefined> {
		...
		const windsurfDir = this.getWindSurfDir();
		if (!windsurfDir || !fs.existsSync(windsurfDir)) return undefined;

		const sessionsDir = path.join(windsurfDir, 'sessions');
	...
	private getWindSurfDir(): string | undefined {
		const home = process.env.HOME || process.env.USERPROFILE || '';
		const directories: Record<string, string> = {
			win32: path.join(process.env.APPDATA || '', 'WindSurf'),
...
export async function analyzeWithWindSurf(projectPath: string): Promise<AnalyzerResult> {
	const analyzer = new WindSurfAnalyzer(projectPath);
